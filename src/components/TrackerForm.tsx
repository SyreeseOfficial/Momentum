...
interface SaveValues {
    ...
    timerIncrement: number | undefined;
}
...
export function TrackerForm({ title, saveLabel, initialValues, onSave, onCancel }: TrackerFormProps) {
    ...
    const [trackerType, setTrackerType] = useState<TrackerType>(initialValues?.trackerType ?? 'count');
    const [timerIncrement, setTimerIncrement] = useState(initialValues?.timerIncrement ?? '30');
